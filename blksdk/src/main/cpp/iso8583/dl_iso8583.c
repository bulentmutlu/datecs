						             ...
						             DL_ISO8583_MSG *ioMsg )
{
	...
	err = DL_ISO8583_MSG_SetField_Bin(iField, iDataStr, (DL_UINT16)DL_STR_StrLen(iDataStr),ioMsg);
...
// returns: error code
DL_ERR DL_ISO8583_MSG_SetField_Bin ( DL_UINT16       iField,const DL_UINT8 *_iData,DL_UINT16 _iDataLen, DL_ISO8583_MSG *ioMsg )
{
	DL_ERR    err = kDL_ERR_NONE;
	DL_UINT8 *ptr = NULL;
...
// returns: error code
DL_ERR DL_ISO8583_MSG_GetField_Str ( DL_UINT16  iField, const DL_ISO8583_MSG  *iMsg, DL_UINT8 **oPtr )
{
	...
	*oNumBytes = 0;
	...
	for ( fieldIdx=0 ; (fieldIdx<MIN(iHandler->fieldItems,kDL_ISO8583_MAX_FIELD_IDX+1)) && !err ; fieldIdx++ )
	{
			 ...
			 DL_ISO8583_IS_BITMAP(iHandler->fieldArr[fieldIdx].fieldType) ) // bitmap
		{
			...
			//IASSERT_MSG(err == 0, "ISO PACK ERROR fieldIdx : %d err : %lu", fieldIdx, err);
			if (err) {
				__android_log_print(ANDROID_LOG_ERROR, "com.blk.techpos", "ISO PACK ERROR fieldIdx : %d err : %lu", fieldIdx, err);
				return err;
			}
...
/******************************************************************************/
//
//void DL_ISO8583_MSG_Dump (const DL_ISO8583_HANDLER *iHandler,
//					       const DL_ISO8583_MSG     *iMsg )
//{
//	DL_UINT16 i;
//
...
//                        {
//                            int dumpLen = (*DebugModuls() & DM_VERBOSE) ? iMsg->field[i].len : MIN(100, iMsg->field[i].len);
...
/*int DL_ISO8583_MSG_getMsg ( const DL_ISO8583_HANDLER *iHandler,	const DL_ISO8583_MSG *iMsg, int MsgNo, unsigned char *Databuf)
{
	DL_UINT16 i = 0;
	...
	for ( i=0 ; i<(iHandler->fieldItems) ; i++ )
	{
		if ( NULL != iMsg->field[i].ptr ) // present 
		{
			DL_ISO8583_FIELD_DEF *fieldDef = DL_ISO8583_GetFieldDef(i,iHandler);
                        if(MsgNo == i)
                        {                                
                                memcpy(Databuf,iMsg->field[i].ptr, iMsg->field[i].len);
                                return iMsg->field[i].len;
                        }
		}
