...
DL_ERR _pack_iso_TRACK ( DL_UINT16                    iField,
						  const DL_ISO8583_MSG        *iMsg,
						  const DL_ISO8583_FIELD_DEF  *iFieldDefPtr,
						  DL_UINT8                   **ioPtr );

DL_ERR _unpack_iso_TRACK ( DL_UINT16                    iField,
...
/* ISO_Z    */ {_unpack_iso_TRACK ,_pack_iso_TRACK },//{_unpack_iso_BINARY,_pack_iso_BINARY},
// ISO_Z     {_unpack_iso_BINARY,_pack_iso_BINARY},   // @bm 20200203
...
unsigned short EP_ascii2bcd(unsigned char *prm_baDestBcd, const char *prm_baSourceAscii, unsigned short len )
{
	int strInx, bcdInx;
	char odd;
	char subValue;
	...
	{
		odd = 1;
		//bcdVal[bcdInx] = strVal[strInx++]-0x30;
		prm_baDestBcd[bcdInx] = 0;
	}

	for (strInx = 0; strInx < len; strInx++) {
		if (prm_baSourceAscii[strInx] >= 'A' && prm_baSourceAscii[strInx] <= 'F')
		{
			subValue = 'A' - 10;
		}
		else if (prm_baSourceAscii[strInx] >= 'a' && prm_baSourceAscii[strInx] <= 'f')
		{
			subValue = 'a' - 10;
		}
		else
		...
		if ((strInx + odd) % 2)
		{
			prm_baDestBcd[bcdInx] <<= 4;
			prm_baDestBcd[bcdInx] |= (prm_baSourceAscii[strInx] - subValue);
						  ...
						  DL_UINT8                   **ioPtr )
{
	...
	if ( !err )
	{
		...
		memset(tmpBuff, 0xFF, 40);

		EP_ascii2bcd(tmpBuff, dataPtr, actLen);                
		tmpLen = actLen / 2;         
		if(actLen % 2) 
                {
			 tmpLen += 1;                         
                    //     EP_printf("35 LEN +1 !:%d\n", tmpLen);
                }
                
                if(tmpLen > 37)
                {
                  //   EP_printf("35 LEN FORCE TO 37:%d\n", tmpLen);
                     tmpLen = 37;                
                }
                                
                //EP_printf("35 Fin data on iso tmpLen:%d\n", tmpLen);
                //EP_HexDump(tmpBuff, tmpLen);

                
		DL_MEM_memcpy(tmpPtr,tmpBuff, tmpLen);
		tmpPtr += tmpLen;
	}
